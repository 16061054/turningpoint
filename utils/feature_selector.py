...
class FeatureSelector():
    ...
    def plot_collinear(self, plot_all = False):
        ...
        sns.heatmap(corr_matrix_plot, cmap=cmap, center=0,
                    linewidths=.35,
                    ) #linewidths=.25 cbar_kws={"shrink": 0.6}, , cbar_kws={"shrink": 0.9}

        # Set the ylabels 
        ax.set_yticks([x + 0.5 for x in list(range(corr_matrix_plot.shape[0]))])
        ax.set_yticklabels(list(corr_matrix_plot.index), size = int(150 / corr_matrix_plot.shape[0])); # 100

        # Set the xlabels 
        ax.set_xticks([x + 0.5 for x in list(range(corr_matrix_plot.shape[1]))])
        ax.set_xticklabels(list(corr_matrix_plot.columns), size = int(150 / corr_matrix_plot.shape[1]));
        plt.title(title, size = 20) # 14
